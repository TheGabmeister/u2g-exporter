...
- FBX files are copied directly, but the exported copy may need `UnitScaleFactor` patched so Godot import scale matches Unity import scale. Never modify the original Unity source FBX in place.
...
5. Verify that any path, transform, material mapping, placeholder behavior, scene-restore logic, FBX-scale handling, and skip-report behavior still match the spec exactly.
