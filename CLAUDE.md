...
Unity and Godot interpret FBX `UnitScaleFactor` metadata differently. Unity's `ModelImporter` has `useFileScale`, `useFileUnits`, `globalScale`, and `fileScale` settings that control how vertex data is scaled at import. Godot's ufbx importer always applies `UnitScaleFactor × 0.01`. The exporter patches the `UnitScaleFactor` in the exported FBX copy so Godot's import matches Unity's visual scale. `PatchUnitScaleFactorForGodot` reads the `ModelImporter` settings, computes the target USF, and overwrites the value in the FBX binary. The original Unity source file is never modified.
