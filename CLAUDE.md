...
### Patch 3: Non-Unit Lcl Scaling Removal

Unity bakes every Model node's `Lcl Scaling` into vertex data on import — the transform never appears in the scene graph. Godot (ufbx) preserves `Lcl Scaling` as a node transform, so any non-unit value causes the mesh to be visually scaled a second time. The exporter resets **all** non-unit `Lcl Scaling` entries to `(1, 1, 1)`. This covers both the Blender-style baked compensation case (`OriginalUSF / USF`) and FBX files that were simply authored with explicit Model-node scaling.
