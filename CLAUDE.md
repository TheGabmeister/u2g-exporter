...
Unity and Godot interpret FBX `UnitScaleFactor` metadata differently. Unity's `ModelImporter` has `useFileScale`, `useFileUnits`, `globalScale`, and `fileScale` settings that control how vertex data is scaled at import. Godot's ufbx importer has its own logic. The exporter uses `ComputeGodotScaleCompensation` to apply a per-instance scale factor on the FBX instance transform. This works correctly because multi-mesh FBX children are written as override nodes (not separate instances), so the scale only applies once at the root.

**Do NOT attempt the `.import` file approach.** Pre-generating Godot `.import` files with `nodes/root_scale` was tried and breaks `.tscn` resource references — Godot requires a complete `[remap]` section (uid, cached import path) that cannot be reliably pre-generated by an external tool. The per-instance transform compensation is the only viable approach.

**Known limitation:** The per-instance compensation over-scales child FBX instances in hierarchical prefabs (e.g., wheels on a car body each get their own ×100 factor).
