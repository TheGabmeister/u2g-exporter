...
- **Godot (ufbx)** always applies the FBX's unit conversion: vertices are scaled by `UnitScaleFactor × 0.01`. For an FBX authored in centimeters (USF=1), this is ×0.01.
...
**Fix:** The exporter patches the `UnitScaleFactor` value in the **exported copy** of the FBX binary so that Godot's import (`vertices × USF × 0.01`) produces the same visual scale as Unity. The target USF is computed from Unity's `ModelImporter` settings:
...
`FbxExporter.PatchUnitScaleFactorForGodot` reads the `ModelImporter` via `AssetImporter.GetAtPath()`, locates the `UnitScaleFactor` double in the FBX binary using the existing parser, and overwrites it in-place. The original Unity source file is never modified — only the exported copy.
