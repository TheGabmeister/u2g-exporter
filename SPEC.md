...
Unity and Godot interpret FBX unit metadata differently when importing the same file. The exporter patches the **exported copy** of the FBX binary (never the Unity source) to fix two independent issues:

#### Issue 1: UnitScaleFactor mismatch

- **Godot (ufbx)** always applies: `vertices × UnitScaleFactor × 0.01`.
- **Unity** behaviour depends on `ModelImporter` settings. With `Use File Scale = false` (common default), Unity ignores the file's unit metadata and uses raw vertex values scaled only by `globalScale` (typically 1.0).

This mismatch means models can appear ~100× too small in Godot compared to Unity.

**Fix:** The exporter patches `UnitScaleFactor` so that `USF × 0.01 = Unity's effective scale`:

```
useFileScale ON  → targetUSF = fileScale × globalScale / 0.01
useFileScale OFF → targetUSF = globalScale / 0.01
```

#### Issue 2: Baked unit conversion scale on Model nodes

Some FBX exporters (notably Blender) bake an `Lcl Scaling` transform on Model nodes to compensate for the difference between `UnitScaleFactor` and `OriginalUnitScaleFactor`. For example, an FBX with USF=100 and OrigUSF=1 may have `Lcl Scaling = (0.01, 0.01, 0.01)` on the mesh Model node. Unity collapses this into vertex data on import, so the model displays correctly. Godot preserves the FBX node hierarchy faithfully, so the 0.01 scale is applied as a node transform, shrinking the model 100×.

**Fix:** When `UnitScaleFactor ≠ OriginalUnitScaleFactor`, the exporter scans for `Lcl Scaling` entries whose uniform value matches the expected compensation ratio (`OriginalUSF / USF`) and resets them to `(1, 1, 1)`.

**Detection criteria:** The patch only triggers when all three conditions are met:
1. `UnitScaleFactor ≠ OriginalUnitScaleFactor` in the FBX metadata
2. An `Lcl Scaling` property exists in the file
3. Its value is a uniform scale exactly matching `OriginalUSF / USF`

Both patches are applied in a single pass by `FbxExporter.PatchFbxForGodot`, which reads `ModelImporter` settings via `AssetImporter.GetAtPath()` and parses the FBX binary to locate and overwrite the relevant values.

**Known limitations:**

- **ASCII FBX files** are not patched. The binary parser safely returns "not found" and the file is copied unmodified, but scale may be wrong in Godot. ASCII FBX is uncommon in modern toolchains.
- **Lcl Scaling false positives** — If an FBX has `USF ≠ OriginalUSF` and a Model node happens to have a legitimate artistic uniform scale that exactly equals `OriginalUSF / USF`, it will be incorrectly reset to 1.0. This coincidence is extremely unlikely in practice.

**Rejected approach — `.import` files:** Pre-generating Godot `.import` files with `nodes/root_scale` was attempted but breaks `.tscn` resource references. Godot's `.import` files require a complete `[remap]` section (including `uid` and cached import `path`) for the resource system to resolve `ExtResource` references in `.tscn` files. Without these, the FBX loads as a broken dependency. Since the `uid` and cache path are generated by Godot's internal import pipeline, they cannot be reliably pre-generated by an external tool.
...
Six shader families are supported:

| Unity Shader | Godot Material | Notes |
|---|---|---|
| `Universal Render Pipeline/Lit` | `StandardMaterial3D` | Full PBR mapping |
| `Universal Render Pipeline/Simple Lit` | `StandardMaterial3D` | Same property names as URP Lit; metallic properties absent but guarded by `HasProperty` checks |
