...
#### Issue 2: Non-unit Lcl Scaling on Model nodes

Unity bakes every Model node's `Lcl Scaling` into vertex data on import — the transform never appears in the scene graph. Godot (ufbx) preserves `Lcl Scaling` as a node transform, so any non-unit value causes the mesh to be visually scaled a second time.

This affects two known cases:
- **Blender-style baked compensation:** FBX exporters bake `Lcl Scaling = OriginalUSF / USF` to cancel out the unit conversion (e.g. `(0.01, 0.01, 0.01)` when USF=100, OrigUSF=1).
- **Authored Model-node scaling:** Some FBX files are authored with explicit non-unit `Lcl Scaling` on mesh Model nodes (e.g. `(10, 10, 10)`), even when `USF == OriginalUSF`. Unity absorbs this into vertices; Godot applies it as a transform.

**Fix:** The exporter resets **all** `Lcl Scaling` entries whose value is not `(1, 1, 1)` back to `(1, 1, 1)`.

All patches are applied in a single pass by `FbxExporter.PatchFbxForGodot`, which reads `ModelImporter` settings via `AssetImporter.GetAtPath()` and parses the FBX binary to locate and overwrite the relevant values.
