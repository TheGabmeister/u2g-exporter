...
**Fix:** The exporter applies a per-instance scale compensation on FBX instance transforms in `.tscn` files. `FbxExporter.ComputeGodotScaleCompensation` parses the FBX binary to extract `UnitScaleFactor`, computes the Godot-side conversion (`USF × 0.01`), and returns `unityLocalScale / godotConversion`. This scale is baked into the instance's `Transform3D` by `PrefabExporter` (root instances) and `NodeConverter` (scene-level instances).
...
**Known limitation:** The per-instance compensation over-scales child FBX instances in hierarchical prefabs (e.g., wheels on a car body each get their own ×100 factor). A correct solution would need to operate at the FBX import level, but this is not feasible with pre-generated `.import` files.
